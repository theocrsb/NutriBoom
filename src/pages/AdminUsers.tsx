...
const Admin = () => {
  ...
  useEffect(() => {
    axios
      .get('http://api-nutriboom.dev-formation.fr/api/users', {
        headers: {
  ...
  const suppAccount = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (
      ...
      axios
        .delete(
          `http://api-nutriboom.dev-formation.fr/api/users/${e.currentTarget.value}`,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem('accesstoken')}`,
            },
  ...
  const validateRole = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (
      ...
      axios
        .patch(
          `http://api-nutriboom.dev-formation.fr/api/users/${e.currentTarget.value}/admin`,
          {
            role: {
              id: valueState,
            },
